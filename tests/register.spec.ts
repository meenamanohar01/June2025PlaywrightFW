...
//Connect with csv file using fs 
// then  readFileSync() reads the entire contents of a file.
//Read csv file in fileContent
let fileContent = fs.readFileSync('./data/register.csv', 'utf-8');

//parse() will read the entire fileContent and returns some unknown[] say I have stored in variable registerationData
//Type of registerationData is of type of Schema that we have defined  i.e RegData
...
/* test(`verify user is able to register`, async ({ page,baseURL }) => {
    let loginPage = new LoginPage(page);
    await loginPage.goToLoginPage(baseURL); //Here baseURL is coming from playwright.config.ts file
    const registerPage: RegisterPage = await loginPage.navigateToRegisterPage();
 let isUserRegsitered : boolean=   await registerPage.registerUser(

        'Priyanka',
        'Singh',
        'meena100rav@gmail.com',
        '+17202751560',
        'test@123456',
        'yes'
    )

    expect(isUserRegsitered).toBeTruthy();
